...
Razor's uses specially modified forks of PHPUnit and JSUnit that don't need PEAR. This eliminates PHPUnit's legendary installation problems for Mac and Windows users. Zero dependencies also means you can check Razor into your team's version control system, confident that it will run on any machine.
...
Razor lets developers script tests that were previously impossible to automate. Its powerful test core can install and activate plugins, load the entire database from an image file, and even check the code coverage of unit tests. The plugin's powerful command-line interface makes it easy to integrate Razor with large automated test systems.
