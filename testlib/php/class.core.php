 ...
 * Runs a user-configurable panel of unit tests against a WordPress plugin installation
 *
 * @version 3.2
 * @since 0.1
...
class RAZ_test_core {
	...
	var $razor_version = "3.2";		    // Version number for Razor
	var $razor_build_date = "2013-01-12";	    // The build date for this version
	...
	var $db_object;				    // The test runner's database object
	...
	var $cmd_options = array(
		...
		"dbname" =>	array(	"required"=>	false,
					...
					"values"=>	null,
					"default"=>	'"unit_test"',
					"example"=>	"--dbname=my_database --expertmode"
				),
		...
		"dbhost" =>	array(	"required"=>	false,
					...
					"values"=>	null,
					"default"=>	'"localhost"',
					"example"=>	"--dbhost=example.com --dbhost=192.168.1.1"
				),
	...
	public function setDatabase() {
		...
		if( array_key_exists('dbname', $this->options) ){
			...
			if( !array_key_exists('expertmode', $this->options) ){
				...
				echo "\n##############################################################################\n";
				echo "\nYou set the database name to be '" . $this->options['dbname'] . "' when you ran the Razor";
	...
	/**
         * Allows razor's testplan classes to register themselves with the test core. We've built
	...
	public function loadTestPlan(){
		...
		// At this point we need the exception classes
		require_once($this->path_test_runner . '/testlib/php/class.utils.exception.php');
		require_once($this->path_test_runner . '/testlib/php/class.utils.exception.handler.php');
	...
	public function loadTestPlatform() {
		...
		require_once($this->path_test_runner . '/testlib/php/class.plugins.php');
		require_once($this->path_test_runner . '/testlib/php/class.filesystem.php');
		require_once($this->path_test_runner . '/testlib/php/class.utils.debug.php');
		require_once($this->path_test_runner . '/testlib/php/class.utils.debug.diff.php');
		require_once($this->path_test_runner . '/testlib/php/class.wp.abstraction.php');
	...
	public function loadTestPanel(){
		...
		catch (RAZ_exception $e){

			echo "\nPROBLEM LOADING TEST PANEL\n";
			$error = RAZ_debug::formatError_print($e->data);
			var_dump($error);
			die;
		}
	...
	/**
         * Sets up the test panel's assets
         *
	 * @version 3.2
	 * @since 0.1
         */
	...
	public function setupAssets() {
		...
		try {
			$this->testpanel->setupAssets();
		}
		catch (RAZ_exception $e){

			echo "\nPROBLEM SETTING UP ASSETS\n";
			$error = RAZ_debug::formatError_print($e->data);
			var_dump($error);
			die;
		}
	...
	public function setupDB() {
		...
		$this->db_object = new RAZ_test_db(DB_USER, DB_PASSWORD, DB_NAME, DB_HOST );

		// Trap db user not existing on the SQL server, or not being allowed to
		// connect from localhost
		if(!$this->db_object->dbh){
			...
			echo " OK - Handle: '" . $this->db_object->dbh . "' \n";
		}

		// Open the unit_test database
		// =================================================================

		echo "Checking if database can be written to...";

		// Trap user existing but database not existing on SQL server
		if( !@mysql_select_db( DB_NAME, $this->db_object->dbh ) ){
		...
		if($this->load_db_image){
			...
			drop_tables($this->db_object);

			echo " OK\n";

			echo "Loading test db into SQL server...";
			load_sql_dump($this->db_object, $this->db_image_file);
			echo " OK\n";
			...
			if($this->remap_folder){
				...
				$remap_ok = call_user_func($this->folder_remap_function, $this->db_object, $stub);
				//$remap_ok = remap_directory($this->db_object, $stub, $error);
	...
	/**
         * Sets the loaded install to the state specified by the test panel
         *
	 * @version 3.2
	 * @since 0.1
         */
	...
	public function setupState() {
		...
		try {
			$this->testpanel->setupState();
		}
		catch (RAZ_exception $e){

			echo "\nPROBLEM SETTING UP STATE\n";
			$error = RAZ_debug::formatError_print($e->data);
			var_dump($error);
			die;
		}
	...
	/**
         * Sets the caching engines to use during tests.
         *
	 * @version 3.2
	 * @since 0.1
         */
	...
	public function setCache() {
		...
		if( !$apc_cli_enabled || $xdebug_enabled ){
		...
		if(!$apc_cli_enabled){
			...
			echo "WARN: APC not installed, or is disabled for CLI scripts.\n";
		...
		// Warn about slowdowns caused by xDebug
		// ====================================================

		if($xdebug_enabled){

			echo "WARN: xDebug is currently active (80% reduction in test speed)\n";
		}
	...
	public function runTests() {
		...
		if( array_key_exists('group', $this->options) ){
			...
			$groups = trim($this->options['group']);
		...
		try {
			...
			echo "\nALL REQUESTED TESTCASES ARE DISABLED\n";
			$error = RAZ_debug::formatError_print($e->data);
			var_dump($error);
			die;
		}
	...
	/**
         * Logs the results from the unit tests
         *
	 * @version 3.2
	 * @since 0.1
         */
	...
	/**
         * Tears-down the test panel's assets
         *
	 * @version 3.2
	 * @since 0.1
         */

	public function tearDownAssets() {

		try {
			$this->testpanel->tearDownAssets();
		}
		catch (RAZ_exception $e){

			echo "\nPROBLEM TEARING DOWN ASSETS\n";
			$error = RAZ_debug::formatError_print($e->data);
			var_dump($error);
			die;
		}
