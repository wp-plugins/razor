<?php
/**
 * RAZOR UNIT TEST PROFILER
 * A simple manually-instrumented profiler for WordPress. Records basic execution time, and a summary of the actions and SQL
 * queries run within each block. Start() and stop() must be called in pairs, for example:
 *
...
class RAZ_Profiler {
	...
	function start($name) {
		...
		// Reset the WP DB queries log, storing it on the profile stack if necessary
		global $wpdb;
...
$bpmp = new RAZ_Profiler();

function bpmp_start($name) {
...
function bpmp_results() {
    
	return $GLOBALS['wppf']->results();
}

function bpmp_print_summary() {
