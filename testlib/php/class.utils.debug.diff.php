...
/**
 * RAZOR DEBUGGING TOOLS - DIFF CLASS
...
class RAZ_fineDiff {
	...
	public static $paragraphGranularity	=   array(  RAZ_fineDiff::paragraphDelimiters);

	public static $sentenceGranularity	=   array( 
							    RAZ_fineDiff::paragraphDelimiters,
							    RAZ_fineDiff::sentenceDelimiters
						    );

	public static $wordGranularity		=   array(	
							    RAZ_fineDiff::paragraphDelimiters,
							    RAZ_fineDiff::sentenceDelimiters,
							    RAZ_fineDiff::wordDelimiters
						    );
	...
	public static $characterGranularity	=   array(	
							    ...
							    RAZ_fineDiff::sentenceDelimiters,
							    RAZ_fineDiff::wordDelimiters,
							    RAZ_fineDiff::characterDelimiters
						    );
	...
	public function __construct($from_text = '', $to_text = '', $granularityStack = null) {
		...
		$this->granularityStack = $granularityStack ? $granularityStack : RAZ_fineDiff::$characterGranularity;
		$this->edits = array();
	...
	public function renderDiffToHTML() {
		...
		foreach( $this->edits as $edit ){
			...
			if ( $edit instanceof RAZ_fineDiffCopyOp ){

				RAZ_fineDiff::renderDiffToHTMLFromOpcode('c', $this->from_text, $in_offset, $n);
			}
			elseif( $edit instanceof RAZ_fineDiffDeleteOp ) {

				RAZ_fineDiff::renderDiffToHTMLFromOpcode('d', $this->from_text, $in_offset, $n);
			}
			elseif( $edit instanceof RAZ_fineDiffInsertOp ) {

				RAZ_fineDiff::renderDiffToHTMLFromOpcode('i', $edit->getText(), 0, $edit->getToLen());
			}
			else { // if ( $edit instanceof RAZ_fineDiffReplaceOp )
	...
	*/
	public static function getDiffOpcodes($from, $to, $granularities = null) {
		...
		$diff = new RAZ_fineDiff($from, $to, $granularities);
	...
	*/
	public static function getDiffOpsFromOpcodes($opcodes) {
		...
		$diffops = new RAZ_fineDiffOps();
		RAZ_fineDiff::renderFromOpcodes(null, $opcodes, array($diffops,'appendOpcode'));
	...
	*/
	public static function renderToTextFromOpcodes($from, $opcodes) {
		...
		ob_start();
		RAZ_fineDiff::renderFromOpcodes($from, $opcodes, array('RAZ_fineDiff','renderToTextFromOpcode'));
	...
	*/
	public static function renderDiffToHTMLFromOpcodes($from, $opcodes) {
		...
		ob_start();
		RAZ_fineDiff::renderFromOpcodes($from, $opcodes, array('RAZ_fineDiff','renderDiffToHTMLFromOpcode'));
	...
	*/
	private function _processGranularity($from_segment, $to_segment) {
		...
		foreach( RAZ_fineDiff::doFragmentDiff($from_segment, $to_segment, $delimiters) as $fragment_edit ){

			// increase granularity
			if( $fragment_edit instanceof RAZ_fineDiffReplaceOp && $has_next_stage ) {
			...
			elseif( $fragment_edit instanceof RAZ_fineDiffCopyOp && $this->last_edit instanceof RAZ_fineDiffCopyOp ){

				$this->edits[count($this->edits)-1]->increase($fragment_edit->getFromLen());
				$this->from_offset += $fragment_edit->getFromLen();
			}
			else {
				/* $fragment_edit instanceof RAZ_fineDiffCopyOp */
				/* $fragment_edit instanceof RAZ_fineDiffDeleteOp */
				/* $fragment_edit instanceof RAZ_fineDiffInsertOp */
	...
	*/
	private static function doFragmentDiff($from_text, $to_text, $delimiters) {
		...
		if( empty($delimiters) ){
			...
			return RAZ_fineDiff::doCharDiff($from_text, $to_text);
		}
		...
		$from_fragments = RAZ_fineDiff::extractFragments($from_text, $delimiters);
		$to_fragments = RAZ_fineDiff::extractFragments($to_text, $delimiters);
		...
		while( $job = array_pop($jobs) ){
			...
			if( !$from_segment_length || !$to_segment_length ){
				...
				if( $from_segment_length ){
					...
					$result[$from_segment_start * 4] = new RAZ_fineDiffDeleteOp($from_segment_length);
				}
				elseif( $to_segment_length ){

					$result[$from_segment_start * 4 + 1] = new RAZ_fineDiffInsertOp(substr($to_text, $to_segment_start, $to_segment_length));
				}
			...
			if( $best_copy_length ){
				...
				$result[$best_from_start * 4 + 2] = new RAZ_fineDiffCopyOp($best_copy_length);
				$jobs[] = array($best_from_start + $best_copy_length, $from_segment_end, $best_to_start + $best_copy_length, $to_segment_end);
			}
			else {
				$result[$from_segment_start * 4 ] = new RAZ_fineDiffReplaceOp($from_segment_length, substr($to_text, $to_segment_start, $to_segment_length));
			}
	...
	private static function doCharDiff($from_text, $to_text) {
		...
		while ( $job = array_pop($jobs) ) {
			...
			if ( !$from_segment_len || !$to_segment_len ) {
				if ( $from_segment_len ) {
					$result[$from_segment_start * 4 + 0] = new RAZ_fineDiffDeleteOp($from_segment_len);
					}
				else if ( $to_segment_len ) {
					$result[$from_segment_start * 4 + 1] = new RAZ_fineDiffInsertOp(substr($to_text, $to_segment_start, $to_segment_len));
					}
				continue;
				}
			if ( $from_segment_len >= $to_segment_len ) {
			...
			if ( $copy_len ) {
				...
				$result[$from_copy_start * 4 + 2] = new RAZ_fineDiffCopyOp($copy_len);
				$jobs[] = array($from_copy_start + $copy_len, $from_segment_end, $to_copy_start + $copy_len, $to_segment_end);
				}
			// no match,  so delete all, insert all
			else {
				$result[$from_segment_start * 4] = new RAZ_fineDiffReplaceOp($from_segment_len, substr($to_text, $to_segment_start, $to_segment_len));
				}
			}
		ksort($result, SORT_NUMERIC);
		return array_values($result);
...
class RAZ_fineDiffDeleteOp extends RAZ_fineDiffOp {
...
class RAZ_fineDiffInsertOp extends RAZ_fineDiffOp {
...
class RAZ_fineDiffReplaceOp extends RAZ_fineDiffOp {
...
class RAZ_fineDiffCopyOp extends RAZ_fineDiffOp {
...
* Collection of ops
*/
class RAZ_fineDiffOps {

	public function appendOpcode($opcode, $from, $from_offset, $from_len) {

		if( $opcode === 'c' ){

			$edits[] = new RAZ_fineDiffCopyOp($from_len);
		}
		else if( $opcode === 'd' ){

			$edits[] = new RAZ_fineDiffDeleteOp($from_len);
		}
		else { // if ( $opcode === 'i' )

			$edits[] = new RAZ_fineDiffInsertOp(substr($from, $from_offset, $from_len));
		}
	}
